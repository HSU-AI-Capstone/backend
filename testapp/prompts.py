...
# 데이터 구조 요구사항[
- points 리스트의 각 항목은 반드시 딕셔너리 형태여야 하며, 'text' 키를 포함해야 합니다.
- 첫 페이지의 points는 [{"text": "주제1"}, {"text": "주제2"}] 형태여야 합니다.
- 내용 페이지의 points는 [{"text": "주제", "explanation": "설명"}] 형태여야 합니다.
- 마지막 페이지의 points는 [{"text": "- 이전 페이지 제목"}] 형태여야 합니다.
- 모든 points 항목은 문자열이 아닌 딕셔너리 형태여야 합니다.
]
...
- 생성하는 코드는 **반드시** 다음 구조를 따라야 해:
    ...
    3. 스타일링 및 헬퍼 함수 정의:
       - set_font 함수는 text_frame을 받아서 폰트를 설정해야 함
       - text_frame.paragraphs를 순회하면서 각 paragraph의 font 속성을 설정
       - paragraph 자체는 paragraphs 속성을 가지지 않으므로 주의
